...
parser.add_argument ('--save_dir', help = 'Provide saving directory. Optional argument, please include .pth', type = str)
parser.add_argument ('--arch', help = 'densenet121 can be used if this argument specified, otherwise resnet50 will be used', type = str)
parser.add_argument ('--lr', help = 'Learning rate, default = 0.003', type = float)
parser.add_argument ('--hidden_units', help = 'Hidden units in Classifier. Default = 512', type = int)
...
parser.add_argument ('--validate_at', help = "print score on vaildation at x forward passes, default 40", type = int)
...
if args.arch:
       model = build_network('resnet50', hidden_layer1_units = args.hidden_units if args.hidden_units else 512)
else:
    model = build_network(hidden_layer1_units = args.hidden_units if args.hidden_units else 512)
            ...
            for images,labels in trainloader:
                ...
                if steps % print_scores_in == 0:
                    ...
                    print('Epoch: {}/{}.'.format(e+1, epochs),
                      'Train loss: {:.3f}.'.format(running_loss/print_scores_in),
                      'Validation Loss: {:.3f}. '.format(test_loss/len(valloader)),
